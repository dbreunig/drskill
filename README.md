...
`list` shows a harness's whole loadout in one table: its skills and, once you have run `--mcp-connect`, its MCP tools. Each row has a `kind` (`skill` or `mcp tool`) and a `suite`. The rows are sorted by suite, so a suite reads as a block: all your superpowers skills together, then the rest, then each server's tools together.
