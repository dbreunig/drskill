...
With zero findings there was nothing to triage, so no false-positive class was found and no lexicon or extraction guard changed as a result of this sweep. But zero findings only validates that the two checks stay silent on skills that do not use the syntax, i.e. no false positives were possible because there was no matching text to run the lexicons against. It provides no positive validation of the extraction-into-lexicon path itself: `injection-shell-dangerous` reuses the `_CRED_STORE`, `_EGRESS`, `_pipe_to_shell`, and encoded-blob patterns from `checks/injection.py`, but those patterns' 2026-07-20 corpus tuning was done against prose and bundled-script text, never against text extracted from `` !`cmd` `` or ```` ```! ```` blocks, and this sweep did not exercise that path against a single real instance. The unit tests in `tests/test_checks_skill_shell.py` are, for now, the only validation that the lexicons behave correctly on extracted command text. Noise accepted by decision: none, since there was no noise to accept. The clean result is recorded here rather than skipped, so a future re-run once a public corpus adopts the syntax has a documented baseline to compare against and can close this validation gap.
