...
def test_dangerous_multi_category_ack(tmp_path):
    ...
    # Initial scan with --ci should fail with exit code 1 (error findings: dangerous)
    r = invoke(tmp_path, "scan", "--ci")
    assert r.exit_code == 1, f"Expected exit code 1 (errors), got {r.exit_code}"
    # Verify both dangerous categories are found
    out = invoke(tmp_path, "scan").output
    assert "injection-shell-dangerous" in out, "Expected dangerous findings in output"
    ...
    # Verify dangerous findings are acked (not in output anymore)
    r2 = invoke(tmp_path, "scan")
    assert "injection-shell-dangerous" not in r2.output, "Dangerous findings should be acked"
    # The fixture also triggers injection-remote-fetch warning (the curl|sh line)
    # and injection-shell-unreviewed note. Ack the warning; notes don't affect --ci exit.
    assert invoke(tmp_path, "ack", "injection-remote-fetch", "doubly").exit_code == 0
