...
def run_checks(
    ...
    from drskill.checks import budget, claude_plugin, duplicates, filesystem, heuristics, injection, lockfile, marketplace, mcp, mcp_injection, mcp_spec, mcp_tools, plugin_spec, query_routing, shadowing, skill_shell, spec  # noqa: F401
...
def run_all(world: World, config: Config, progress=None) -> list[Finding]:
    # Import registers every check module exactly once.
    from drskill.checks import budget, claude_plugin, duplicates, filesystem, heuristics, injection, lockfile, marketplace, mcp, mcp_injection, mcp_tools, query_routing, shadowing, skill_shell, spec  # noqa: F401
